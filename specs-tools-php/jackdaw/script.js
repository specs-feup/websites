(function () {
     ...
     */
    var RUN_COMPILER_PATH = 'runJackdaw.php';
    var BASE_CODE_EXAMPLE_URL = 'js/';
    var BASE_LARA_EXAMPLE_URL = 'lara/';

    var ACE_EDITOR_LANGUAGE = "lara";

    var CODE_HOME_EXAMPLE = "navier-stokes.js";
    var LARA_HOME_EXAMPLE = "CallGraph.lara";
    ...
    var runCommand = function (path, outputMode, args) {
            ...
            if (runRequest.readyState === 4) {
                if (runRequest.status === 200) {
                    ...
                    try {
                        ...
                        populateResults(obj);
                        //loadWeaverResult(obj.output);
                        loadWeaverResult("output.js");
                    ...
                    consoleOut.setValue('Could not run Jackdaw.\n' + runRequest.status + ": " + runRequest.statusText);
                }
                endWaitForResults();
            }
        };
        ...
        try {
            ...
            consoleOut.setValue('Could not run Jackdaw. Got exception: ' + e.name);
            endWaitForResults();
        }
    };
    ...
    //Save changes in browser for Jackdaw
    var jackdawCodeCache = new PreserveCode("jackdaw");
    if (!jackdawCodeCache.hasSavedContent()) {
        loadLaraExample(LARA_HOME_EXAMPLE);
        loadCodeExample(CODE_HOME_EXAMPLE);
    }

    /***************************LS***************************/
    var BASE_LS_JSON_URL = 'https://raw.githubusercontent.com/specs-feup/jsweaver/master/jsweaver/src/pt/up/fe/specs/jackdaw/JackdawWeaver.json';
